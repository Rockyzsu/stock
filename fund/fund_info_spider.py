...
class FundSpider(object):
    ...
    def create_tb(self):
        create_table = 'create table if not EXISTS `{}` (`基金代码` varchar(20) PRIMARY KEY,`基金简称` varchar(100),`最新规模-万` float,`实时价格` float,`涨跌幅` float,`成交额-万` float,`净值日期` VARCHAR(10),`单位净值` float,`累计净值` float,`折溢价率` float ,`申购状态` VARCHAR(20),`申赎状态` varchar(20),`基金经理` VARCHAR(200),`成立日期` VARCHAR(20), `管理人名称` VARCHAR(200),`实时估值` INT,`更新时间` VARCHAR(20));'.format(
            today)
        try:
    ...
    def notify(self, today):
        ...
        if now.hour > NOTIFY_HOUR:
            # 下午才会发通知
            query_sql = '''select `基金代码`,`基金简称`,`实时价格`,`实时净值`,`溢价率` from `{}` where `申购状态`='开放' and `申赎状态`='开放' and !(`实时价格`=1 and `涨跌幅`=0 and `成交额-万`=0) order by `溢价率` limit 10'''.format(
                today)
            cursor.execute(query_sql)
            ret = cursor.fetchall()
            html = '<div><table border="1">' \
                   '<tr><th>基金代码</th><th>基金简称</th><th>实时价格</th><th>实时净值</th><th>溢价率</th></tr>'
            for i in ret:
                html += f'<tr><td>{i[0]}</td><td>{i[1]}</td><td>{i[2]}</td><td>{i[3]}</td><td>{i[4]}</td></tr>'
            html += '</table></div>'

            query_sql = '''select `基金代码`,`基金简称`,`实时价格`,`实时净值`,`溢价率` from `{}` where `申购状态`='开放' and `申赎状态`='开放'  and `溢价率` is not null and !(`实时价格`=1 and `涨跌幅`=0 and `成交额-万`=0) order by `折溢价率` desc limit 10'''.format(
                today)
            cursor.execute(query_sql)
            ret = cursor.fetchall()
            html += '<div><table border="1">' \
